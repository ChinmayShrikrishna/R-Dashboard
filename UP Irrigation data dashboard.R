...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("area","Choose type of area:",
                  choices= c("Geographical area","Cultivable area","Net Sown Area ")),
      selectInput("Groundwaterlevel", "Choose time duration:",
                  choices = c("Pre Monsoon","Post Monsoon"))
    ),
    mainPanel(
...
server <- function(input, output) {
  ...
  output$barPlot <- renderPlotly({
    ...
    column <- switch(input$area,
                     "Geographical area" = "geographical_area",
                     "Cultivable area" = "cultivable_area",
                     "Net Sown Area" = "net_sown_area"),
    column <- switch(input$Groundwaterlevel,
                     "Pre Monsoon" = "Pre_monsoon_avg_ground_water_level",
                     "Post Monsoon"="Post_monsoonavg_ground_water_level") 
      case = action
    )
